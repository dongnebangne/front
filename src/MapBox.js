...
const descriptions = {
    범죄주의구간: `- 최근 1년 전체(5대 범죄) 범죄발생현황을 밀도분석 한 정보입니다.<br/> - 경찰청에서 제공받은 밀도분석 정보를 도로상에 등급(10등급)으로 표현하고 있습니다.`,
    자취촌범죄주의구간: `- 추후에 추가 예정입니다.`,
    노인대상범죄주의구간: `- 노인(65세 이상)을 대상으로 한 범죄발생현황을 밀도분석 한 정보입니다.<br/> - 경찰청에서 제공받은 밀도분석 정보를 도로상에 등급(10등급)으로 표현하고 있습니다.`,
    어린이대상범죄주의구간: `- 어린이(13세 이하)를 대상으로 한 범죄발생현황을 밀도분석 한 정보입니다.<br/> - 경찰청에서 제공받은 밀도분석 정보를 도로상에 등급(10등급)으로 표현하고 있습니다.`,
    치안사고통계: `- 최근 3년 전체(9대 범죄) 범죄발생건수를 표시한 정보입니다.<br/> - 경찰관서(경찰서·지구대·파출소)별 발생건수를 등급(5등급)으로 표현하고 있습니다.<br/> - 치안사고통계정보는 발생 건 수가 직접 노출되지 않도록  *등급화* 를 적용하여 표시합니다.`,
    여성밤길치안안전: `- 밤 시간대(20~24시) 전체(5대 범죄) 범죄발생현황을 밀도분석 한 정보입니다.<br/> - 경찰청에서 제공받은 밀도분석 정보를 도로상에 등급(10등급)으로 표현하고 있습니다.<br/><br/> * 지도상에 음영처리 된 부분은 치안안전시설(예. CCTV, 가로등, 방범등, 편의점 등)이 위치하지 않는 지역입니다.`,
};
...
const MapBox = ({ setLayers, setGeojsonVisible, selectedButton, setSelectedButton }) => {
    ...
    const resetLayers = () => {
        ...
        setGeojsonVisible(''); // GeoJSON 레이어 비활성화
    };
    ...
    const handleOptionChange = async (event) => {
        ...
        setSelectedOption(selectedSubcategory);

        // GeoJSON 레이어를 선택된 옵션에 따라 표시
        if (modalTitle === '자취촌범죄주의구간') {
            switch (selectedSubcategory) {
                case '전체':
                    setGeojsonVisible('CRALL');
                    break;
                case '폭력':
                    setGeojsonVisible('CRVIOL');
                    break;
                case '절도':
                    setGeojsonVisible('CRTHEFT');
                    break;
                case '성폭력':
                    setGeojsonVisible('CRSEX');
                    break;
                default:
                    setGeojsonVisible('');
            }
            ...
            try {
                const layers = await getWMSLayer(modalTitle, selectedSubcategory);
                setLayers(layers);
